...
import { Subscription, SubscriptionPlan, User, SubscriptionTransaction, PromoCode } from "@/api/entities";
...
export default function SubscriptionPage() {
      ...
      try {
        ...
        if (currentUser && !['admin', 'super_admin'].includes(currentUser.app_role)) {
          try {
            ...
            console.log("No active subscription"); // Changed log message
            if (isMounted && !abortController.signal.aborted) {
        ...
        if (!isMounted || abortController.signal.aborted) return;
        console.error("Loading subscriptions in guest mode:", error); // Changed message and type
        setUser(null); // Ensure user is null on error
        setSubscriptionPlans(samplePlans); // Show sample plans on error
  ...
  const handlePaymentSuccess = async (plan, cycle, paymentData) => {
    ...
    try {
      ...
      window.location.reload(); // Changed from await loadData()
