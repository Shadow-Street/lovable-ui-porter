...
export default function EventsPage() {
  ...
  // Ref to track if the component is mounted, for imperative async calls
  ...
  // Define loadData function at component level so it can be called from handlers and useEffect
  const loadData = useCallback(async (abortSignal = null) => {
    setIsLoading(true); // Always set loading to true when starting to load data

    try {
      // Try to get user, but allow null for guests
      const currentUser = await User.me().catch(() => null);

      if (abortSignal?.aborted || !componentIsMounted.current) return;
      ...
      }, '-event_date', 50, abortSignal).catch(() => []);
      ...
      if (currentUser?.id) {
        const [ticketsData, attendanceData, subscriptionData] = await Promise.all([
          EventTicket.filter({ user_id: currentUser.id }, null, null, abortSignal).catch(() => []),
          EventAttendee.filter({ user_id: currentUser.id }, null, null, abortSignal).catch(() => []),
          Subscription.filter({ user_id: currentUser.id, status: 'active' }, null, null, abortSignal).catch(() => [])
        ]);
    ...
    setUser, setEvents, setUserTickets, setUserAttendance, setUserSubscription,
    setHasSubscription, setIsLoading, toast, componentIsMounted
  ]);
